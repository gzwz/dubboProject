...
public class LogAspect {
    ...
    private void log(JoinPoint point, Object obj) {
        ...
        if(sign.equals("Start")) {
            ...
            log.info(LogInfo.OR.getName() + ": " + interfaceName + "." + methodNam + "(" + methodName + ") - 调用开始");
            for(Object param : point.getArgs()) {
                sb.append(param + " ");
            }
            log.info("接口入参输出：" + sb.toString());
        } else if(sign.equals("End")) {
            /* 打印出参*/
           /* log.info(LogInfo.OR.getName() + ": " + interfaceName + "." + methodNam + "(" + methodName + ") - 调用结束");
            for(Object param : point.getArgs()) {
                sb.append(param);
            }
            log.info("接口出参输出：" + sb.toString());*/
        } else {
            /* 打印异常信息*/
            log.error(LogInfo.OR.getName() + interfaceName + "接口发生异常：", obj);
        }
    }
}
